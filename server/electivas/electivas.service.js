...
const cuenta = config.airtable.cuentas.filter(Cuenta => Cuenta.id === 'metas').pop();
...
async function obtenerElectivasDesdeAirtable(carreraElegida) {
  return new Promise((resolve, reject) => {
    let result = [];
      baseConnection(carreraElegida).select({view: "Grid view"}).eachPage((records, fetchNextPage) => {
        result = records
          ...
          .map(Electiva => ({
            ...
            horarios: Electiva["Horarios"] ? Electiva["Horarios"].split("\n") : [],
            ...
            comentarios: Electiva["Comentarios"] ? Electiva["Comentarios"].split("\n") : [],
          }));
        fetchNextPage();
      }, (error) => {
        if(error) {
