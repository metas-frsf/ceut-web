...
export class ElectivasComponent implements OnInit {
  ...
  carreras: Carrera[] = [
    {
      ...
      horasTotales: 0,
      airTable: 'Básicas',
      contacto: {instagram: 'metas.frsf', messenger: 'metas.frsf', whatsapp: ''}
    },
    {
      id: 'civil',
      ...
      horasTotales: 14,
      airTable: 'Civil',
      contacto: {instagram: 'consejerosdecivil', messenger: 'consejeros.civil', whatsapp: ''}
    },
    {
      id: 'electrica',
      iconClass: 'fa-bolt',
      ...
      horasTotales: 20,
      airTable: 'Eléctrica',
      contacto: {instagram: 'consejeroselectrica', messenger: 'consejeroselectrica.metas', whatsapp: ''}
    },
    {
      id: 'industrial',
      ...
      horasTotales: 10,
      airTable: 'Industrial',
      contacto: {instagram: 'industrialconsejeros', messenger: 'consejeroselectrica.metas', whatsapp: ''}
    },
    {
      id: 'mecanica',
      iconClass: 'fa-cogs',
      ...
      horasTotales: 10,
      airTable: 'Mecánica',
      contacto: {instagram: 'mecanicametas', messenger: 'metas.mecanica.frsf', whatsapp: ''}
    },
    {
      id: 'sistemas',
      iconClass: 'fa-laptop',
      ...
      horasTotales: 44,
      airTable: 'Sistemas',
      contacto: {instagram: 'ps_metas.frsf', messenger: 'psutnfrsf', whatsapp: ''}
    }
  ];
  ...
  constructor(@Inject(DOCUMENT) private document: any, private electivasService: ElectivasService, private globalService: GlobalService) {
  ...
  calcularCursadas() {
    const horasElectivasCursadas = this.electivas.filter(Electiva => Electiva.cursada).map(Electiva => Electiva.creditos);
    this.contadorCursadas = horasElectivasCursadas.length ? horasElectivasCursadas.reduce((Suma, Horas) => Suma + Horas) : 0;
  }

  calcularAprobadas() {
    const horasElectivasAprobadas = this.electivas.filter(Electiva => Electiva.aprobada).map(Electiva => Electiva.creditos);
    this.contadorAprobadas = horasElectivasAprobadas.length ? horasElectivasAprobadas.reduce((Suma, Horas) => Suma + Horas) : 0;
  }
  ...
  filtrarPorCuatrimestre(filtro: Periodo) {
    ...
    if (filtro.primero) {
      this.electivasFiltradas = this.electivasFiltradas.concat(this.electivas.filter(Electiva => Electiva.cuatrimestre === '1er Cuatrimestre'));
    }
    if (filtro.segundo) {
      this.electivasFiltradas = this.electivasFiltradas.concat(this.electivas.filter(Electiva => Electiva.cuatrimestre === '2do Cuatrimestre'));
    }
    if (filtro.anual) {
      this.electivasFiltradas = this.electivasFiltradas.concat(this.electivas.filter(Electiva => Electiva.cuatrimestre === 'Anual'));
    }
